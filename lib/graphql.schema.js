...
const _getItemsQueryFromEntity = entity => {
  ...
  return {
    ...
    resolve: (obj, args, ctx) => {
      ...
      const order = args.sort ? args.sort : 'id';
  ...
  return {
    ...
    resolve: (obj, args, ctx) => {
      if (args.id) {
        ...
        return ctx.context
          .getObject(entity.name, { where: args.filter })
          .then(data => data);
      }
    }
  };
...
let _filterObjectTypeCache = {};

const _makeFilterObjectType = entity => {
  if (_filterObjectTypeCache[entity.name]) {
    return _filterObjectTypeCache[entity.name];
  }
  let fields = {
  ...
  _filterObjectTypeCache[entity.name] = type;
  return type;
};
