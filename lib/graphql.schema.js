...
const _getItemsQueryFromEntity = entity => {
  return {
    ...
    resolve: (obj, args, ctx) => {
      ...
      return ctx.context.getObjects(entity.name, { limit, offset });
    }
  };
};
...
const _getSchemaFieldsFromEntityWithoutRelationship = ( entity, showId = true) => {
  let fields = {};
  if(showId){
    fields.id ={
...
const _updateEntity = entity => {
  ...
  return {
    ...
    resolve: (obj, args, ctx) => {
      ...
      const promiseUpdateObject = ctx.context.getObjectWithId(entity.name, args.input.id)
        .then(data => {
