...
const getSchemaFromModel = model => {
  ...
  return new GraphQLSchema({
    ...
    mutation: new GraphQLObjectType({
      ...
      // fields: {
        // deleteCompany: {
        //   type: _getObjectTypeFromEntity(model.entities['Company']),
        //   args: {
        //     id: {
        //       type: GraphQLInt,
        //     }
        ...
        //     const temp = ctx.context.getObjectWithId('Company', args.id);
        //     temp.then(data => {
        //       ctx.context.deleteObject(data);
        //     });
        //     return temp;
        //     // return ctx.context.getObjectWithId('Company', args.id).then( data => {
        //     //   ctx.context.deleteObject(data);
        //     // });
        //   }
        // },
        ...
        //     return ctx.context.create('Company', args.company);
        //   }
        // },
      //   ...mutations,
      // }
...
const _getItemQueryByIdFromEntity = (entity) => {
  return {
    ...
    resolve: (obj, args, ctx) => {
      return ctx.context.getObjectWithId(entity.name, args.id).then( data => data);
    }
  }
}
...
const _getSchemaFieldsFromEntityWithoutRelationship = entity => {
  let fields = {
    id: {
      type:GraphQLInt
    }
...
const _deleteEntityById = entity => {
  return {
    ...
    resolve: (obj, args, ctx) => {
      const promiseGetObject = ctx.context.getObjectWithId(entity.name, args.id);
